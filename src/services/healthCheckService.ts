  ...
  while (true) {
    ...
    try {
      ...
      const healthChecks = Object.entries(services).map(
        async ([id, service]) => {
          log(
            `Checking service ${id} (${service.name}) at ${service.checkURL}`,
          );
          ...
          try {
            if (service.checkType === 'http') {
              ...
              if (previousStatus !== service.status) {
                log(
                  `${id} (${service.name}) status changed: ${previousStatus} -> ${service.status}`,
                );
              }

              // Log response details
              log(
                `${id} (${service.name}) responded with status ${response.status}`,
              );
            } else {
              service.status = 'unimplemented check type';
              log(
                `${id} (${service.name}) has unimplemented check type: ${service.checkType}`,
              );
            }
          } catch (error) {
      ...
      log(
        `=== Health check iteration #${checkCount} completed in ${duration}ms ===\n`,
      );
    } catch (error) {
      log(
        `Error during health check iteration #${checkCount}: ${error.message}`,
      );
    }
